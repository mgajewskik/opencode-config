...
const defaultWorktreeConfigText = `{
...
function mergeStringArrays(globalValues: string[], localValues: string[]): string[] {
...
/**
 * Load worktree configuration from global and repo-local config files.
...
async function loadWorktreeConfig(directory: string, log: Logger): Promise<WorktreeConfig> {
	...
	if (globalConfig && localConfig) {
		log.info(`[worktree] Loaded merged config: ${globalConfigPath} + ${localConfigPath}`)
		return mergeWorktreeConfigs(globalConfig, localConfig)
	}
