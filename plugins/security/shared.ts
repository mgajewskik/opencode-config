...
const BASH_DENY_PATTERNS: DenialPattern[] = [
  ...
  rx(String.raw`${TOKEN}docker${ARG}(exec|inspect)${END}`, "container exec or inspect"),
  ...
  rx(String.raw`${TOKEN}(kubectl|k)\b[^;&|]*\s+(exec|drain)${END}`, "dangerous kubectl operation"),
...
const PROTECTED_PATH_PATTERNS: DenialPattern[] = [
  ...
  rx(String.raw`(^|[^A-Za-z0-9_.-])\.config/opencode/auth[^/]*($|[^A-Za-z0-9_./-])`, "OpenCode auth state"),
  rx(String.raw`(^|[^A-Za-z0-9_.-])\.local/share/opencode/auth[^/]*($|[^A-Za-z0-9_./-])`, "OpenCode auth state"),
]
