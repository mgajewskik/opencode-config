...
1. After local validation, review the integrated result once in fresh context with a full packet (exact **criteria and anti-criteria**, changed paths, evidence): `generic` with `LANE: review`. Do not schedule per-file or intermediate reviews unless an early review can prevent expensive rework or a risky implementation decision.
   - For the `smart` workflow only, use its model-bound reviewer (Astra high by default) with a complete free-form briefing instead of `generic` and its packet syntax. The same independence, criteria, evidence, blocker, and PASS/waiver rules apply; this exception does not change other agents' delegation contracts.
